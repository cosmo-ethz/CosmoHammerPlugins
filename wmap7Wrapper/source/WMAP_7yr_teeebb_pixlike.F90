...
!===========================================
  ...
  INCLUDE 'likelihood_v4p1/read_archive_map.fh'
  Include 'likelihood_v4p1/read_fits.fh'
...
if ( use_gibbs_pol_cleaning ) then
  eebbdir = trim(WMAP_data_dir)//'lowlP/gibbs/'
  filename(0)=trim(eebbdir)//'gibbs_masked_ee_ninvplninv_qu_r3_7yr.fits'
  filename(1)=trim(eebbdir)//'gibbs_masked_bb_ninvplninv_qu_r3_7yr.fits'
  filename(2)=trim(eebbdir)//'gibbs_masked_ninv_qu_r3_7yr.fits'
  filename(3)=trim(eebbdir)//'gibbs_wt_r3_7yr.map_q'
  filename(4)=trim(eebbdir)//'gibbs_wt_r3_7yr.map_u'
  filename(6)=trim(eebbdir)//'gibbs_masked_ninvy_e_qu_r3_7yr.fits'
else
...
!=======================================
  ...
  REAL :: Omega_pix
! DH10: add accumulators for parallel loops
  real(8) :: p_r3acc1,p_r3acc2
...
! DH10: OMP parallized loop
!call system_clock(count1, count_rate)
!$OMP PARALLEL DO DEFAULT(SHARED),PRIVATE(p_r3acc1,p_r3acc2,i),SCHEDULE(GUIDED)
  do ip=0,mp-1
     i = 3
     p_r3acc1=0d0
     p_r3acc2=0d0
...
!print *,"time for profile=",real(count2 - count1)/real(count_rate)
! DH10
