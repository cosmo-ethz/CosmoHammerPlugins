...
! ===========================================================================
  ...
  implicit none
  private
  public :: eebb_lowl_like_setup, eebb_lowl_likelihood, teeebb_pixlike_dof

  integer :: nsmax,nlmax,np,mp
#ifdef OPTIMIZE
  REAL, allocatable, dimension(:) :: xxx, yyy
  REAL, allocatable, dimension(:,:) :: ninvplninv2
  ...
  REAL, allocatable, dimension(:,:) :: Dp0
  REAL, allocatable, dimension(:) :: m_r3,w_r3,p_r3,f_r3,zzz
...
!===========================================
subroutine eebb_lowl_like_setup
!===========================================
  ...
  INTEGER :: ReadStatus
  CHARACTER(len=512) :: qfile,ufile,maskfile,filename(0:9),eebbdir
...
#ifdef TIMING
	call wmap_timing_start( 'eebb_lowl_like_setup' )
#endif
...
!-----------------------------
  ...
  eebbdir = WMAP_data_dir

  filename(0)=trim(eebbdir)//'ninv/masked_ee_ninvplninv_qu_r3_p2_norm_corrected_3yr_pxim.QV.fits'
  filename(1)=trim(eebbdir)//'ninv/masked_bb_ninvplninv_qu_r3_p2_norm_corrected_3yr_pxim.QV.fits'
  filename(2)=trim(eebbdir)//'ninv/masked_ninv_qu_r3_p2_norm_corrected_3yr_pxim.QV.fits'
  filename(3)=trim(eebbdir)//'maps/wt_r3_p2_3yr_pxim.QV.map_q'
  filename(4)=trim(eebbdir)//'maps/wt_r3_p2_3yr_pxim.QV.map_u'
  !!$ filename(5)=trim(eebbdir)//'alm/alm_tt_fs_r9_ilc_nopixwin_3yr.dat' ! alm^tt from old ILC 
  filename(5)=trim(eebbdir)//'alm/alm_tt_fs_r9_ilc_nopixwin_3yr_v2.dat'
  filename(6)=trim(eebbdir)//'ninv/masked_ninvy_qu_r3_p2_norm_corrected_3yr_pxim.QV.fits'
  filename(7)=trim(eebbdir)//'maps/wt_r3_p2_3yr_pxim.K1.QV.map_q'
  filename(8)=trim(eebbdir)//'maps/wt_r3_p2_3yr_pxim.K1.QV.map_u'
  filename(9)=trim(eebbdir)//'healpix_data/pixel_window_n0008.txt'
...
!------------------------------
  ...
  maskfile=trim(eebbdir)//'mask/mask_r3_p06.fits'
  inquire(file= maskfile,exist = yes)
  if(.not.yes)then
     write(*,*)"eebb maskfile not found",trim(maskfile)
  endif
...
end subroutine eebb_lowl_like_setup
...
!===========================================
subroutine eebb_lowl_likelihood(nlmaxin,clttin,cltein,cleein,clbbin,chisq_r3,lndet)
!=======================================
...
#ifndef OPTIMIZE
  REAL, allocatable, dimension(:,:) :: NinvSNinv
  real,allocatable,dimension(:,:) ::CQU
#endif
  REAL, external :: SDOT
  REAL, allocatable, dimension(:,:) :: Dp 
  real,allocatable,dimension(:) ::clee,clbb,cltt,clte
  INTEGER :: ip,jp,nlmax,info,l,m,i,k
  integer :: t_start,t_end,crate,cmax
  REAL :: Omega_pix

#ifdef TIMING
	call wmap_timing_start( 'eebb_lowl_likelihood' )
#endif

  nlmax = 23
  Omega_pix = 3.14159/(3.*8.**2.)
  if(nlmaxin.ne.nlmax)then
     write(*,*)"need nlmax",nlmax,", in eebb likelihood, currently",nlmaxin
     stop
  endif
  ...
  lndet = 0d0
	...
	call SGEMV( 'N', mp*(2*mp+1), 2*(nlmax-1), 1., ninvplninv2, &
		mp*(2*mp+1), xxx, 1, 0., yyy, 1 )

	k = 1
	do ip = 0,2*mp-1
	do jp = ip,2*mp-1
		Dp(ip,jp) = Dp0(ip,jp) + yyy(k) &
			+ sig_temp**2*f_r3(ip)*f_r3(jp)/Omega_pix
		k = k + 1
	end do
	end do
...
!
  ...
  do ip=0,mp-1
     ...
     do jp=ip,mp-1
        NinvSNinv(ip,jp) = &
             +SDOT(nlmax-1,clee(2:)-clte(2:)**2./cltt(2:),1,ninvplninv(2:,ngood(ip),ngood(jp),1),1) &
             +SDOT(nlmax-1,clbb(2:),1,ninvplninv(2:,ngood(ip),ngood(jp),2),1) &
             +sig_temp**2.*f_r3(ip)*f_r3(jp)/Omega_pix 
        
        NinvSNinv(ip,mp+jp) = &
             +SDOT(nlmax-1,clee(2:)-clte(2:)**2./cltt(2:),1,ninvplninv(2:,ngood(ip),np+ngood(jp),1),1)&
             +SDOT(nlmax-1,clbb(2:),1,ninvplninv(2:,ngood(ip),np+ngood(jp),2),1)  &
             +sig_temp**2.*f_r3(ip)*f_r3(mp+jp)/Omega_pix 

        NinvSNinv(mp+ip,mp+jp) = &
             +SDOT(nlmax-1,clee(2:)-clte(2:)**2./cltt(2:),1,ninvplninv(2:,np+ngood(ip),np+ngood(jp),1),1) &
             +SDOT(nlmax-1,clbb(2:),1,ninvplninv(2:,np+ngood(ip),np+ngood(jp),2),1) &
             +sig_temp**2.*f_r3(mp+ip)*f_r3(mp+jp)/Omega_pix 
     enddo
  ...
  CALL SPOTRF('U',2*mp,Dp,2*mp,info) 
  IF(info.NE.0)then
	call wmap_likelihood_error( 'eebb: bad spotrf', info )
	chisq_r3 = 0d0
	lndet = 0d0
	return
  endif
...
#ifndef OPTIMIZE
   CALL SPOTRI('U',2*mp,Dp,2*mp,info)
#endif

#ifdef TIMING
	call wmap_timing_checkpoint( 'finished spotri' )
#endif
 
  IF(info.NE.0)then
	call wmap_likelihood_error( 'eebb: bad spotri', info )
	chisq_r3 = 0d0
	lndet = 0d0
	return
  endif
  ...
  enddo
	...
	m_r3 = w_r3-p_r3
	call SPOTRS( 'U', 2*mp, 1, Dp, 2*mp, m_r3, 2*mp, info )
	!chisq_r3 = SDOT(2*mp,m_r3,1,w_r3-p_r3,1) 
	zzz = w_r3-p_r3
	chisq_r3 = sum( m_r3*zzz )
#else

  CALL SSYMV('U',2*mp,1.,CQU(:,:),2*mp,w_r3-p_r3,1,0.,m_r3,1)
  chisq_r3 = SDOT(2*mp,m_r3,1,w_r3-p_r3,1) 
  ...
  deallocate( Dp )
  deallocate( cltt, clte, clee, clbb )
  
#ifdef TIMING
	call wmap_timing_end()
#endif

END subroutine EEBB_LOWL_LIKELIHOOD

end module WMAP_eebb_lowl
