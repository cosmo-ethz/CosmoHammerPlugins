...
import wmap9Wrapper
from wmap9Wrapper.WmapLikelihoodModule import WmapLikelihoodModule
...
class WmapExtLikelihoodModule(WmapLikelihoodModule):
    ...
    """
    def __init__(self, aszIndex=6, path=None, **kwargs):
        """
        Constructor
        """
        super(WmapExtLikelihoodModule, self).__init__(path, **kwargs)
        self.aszIndex = aszIndex

    def setup(self):
        super(WmapExtLikelihoodModule, self).setup()
        
        path = resource_filename(wmap9Wrapper.__name__, FILE_PATH)
    ...
    def postProcessPowerSpectrum(self, ctx):
        ...
        cl_tt = ctx.get(wmap9Wrapper.CL_TT_KEY)
        p = ctx.getParams()
